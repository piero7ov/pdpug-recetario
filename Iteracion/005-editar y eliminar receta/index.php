    ...
    if (!$receta) {
      ...
      if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if (empty($errors)) {
          ...
          // Update receta (mantenemos slug fijo)
...
/* =========================
   ELIMINAR RECETA (GET confirma / POST elimina)
   ========================= */
elseif ($page === "eliminar" && $slug !== "") {
  ...
  try {
    ...
    if (!$receta) {
      ...
      if ($_SERVER["REQUEST_METHOD"] === "POST") {
        // eliminar receta (cascade elimina ingredientes y pasos)
        $db->begin_transaction();
