<?php
/**
 * pdpug.php (PDpug v2.1)
 * FIX IMPORTANTE:
 * - Ahora el stack guarda [tag, level]
 * - Al leer una nueva línea de nivel L, cerramos tags con level >= L
 *   => así un <a> se cierra antes de un <p> hermano y ya no “arrastra” el link.
 *
 * Features:
 * - @include "ruta.pdpug"
 * - @foreach x as y
...
function pdpug_render(string $filePath, array $data = []): string {
  ...
  // 2) Render
  $out = "";
  $stack = []; // cada item: ["tag" => string, "level" => int]

  pdpug_render_lines($lines, $data, $baseDir, $stack, $out);

  // cierre final (cierra todo)
...
function pdpug_render_lines(array $lines, array $data, string $baseDir, array &$stack, string &$out): void {
  ...
  while ($i < $n) {
    ...
    // FIX: cerrar tags al mismo nivel o superior (para hermanos)
    ...
    if (preg_match('/^@if\s+(\$?[a-zA-Z_][a-zA-Z0-9_.]*)\s*$/', $line, $m)) {
      ...
      $cond = pdpug_truthy($val);

      $chosen = $cond ? $trueBlock : $elseBlock;

      if (!empty($chosen)) {
        pdpug_render_lines($chosen, $data, $baseDir, $stack, $out);
        // cerrar lo abierto dentro del bloque (>= nivel del bloque)
    ...
    // guardamos tag + nivel real de indentación
...
/**
 * Cierra tags cuyo level sea >= $levelObjetivo
 * - Si viene un hermano al mismo nivel, cerramos el anterior.
 * - Si bajamos de nivel, cerramos los superiores.
 */
